<?php
    ...
    include_once 'db.php';
     if ($_GET['news_id']) {
        $newsID = $_GET['news_id'];
        $sql_getNewsInfor = "select * from news join news_cate on news.news_cateID=news_cate.news_cateID where news_id=$newsID";
        $result_getNewsInfor = $conn->query($sql_getNewsInfor)->fetch();

        $sql_showNewsCate = "select * from news_cate";
        $result_showNewsCate = $conn->query($sql_showNewsCate);
    }else {
    ...
    <body>
        <div class="wrapper">
            ...
            <div class="main">
                ...
                <div class="main-body">
                    <h1 class="text-center my-3">Cập nhập tin tức</h1>
                    <form class="border p-4 rounded" action="" method="POST" enctype="multipart/form-data">
                        <div class="form-group">
                            <label for="name">Tiêu đề</label>
                            <input class="form-control" type="text" name="news_title" value="<?php echo $result_getNewsInfor['news_title']?>" >
                        </div>
                        <div class="form-group">
                            <label for="file">Hình ảnh</label><br>
                            <img src="asset/images/news/<?php echo $result_getNewsInfor['news_images']?>" style="width: 200px; margin-bottom: 10px;" alt=""><br>
                            <input type="file" id="file" name="news_img" class="form-control-file">
                        </div>
                        <div class="form-group">
                            <label for="name">Giới thiệu</label>
                            <input class="form-control" type="text" name="news_intro" value="<?php echo $result_getNewsInfor['news_intro']?>">
                        </div>
                        <div class="form-group">
                            <label for="name">Nội dung</label>
                            <textarea name="news_content" id="editor" cols="30" rows="10"><?php echo $result_getNewsInfor['news_content']?></textarea>
                        </div>
                        <div class="form-group">
                            <label for="name">Tác giả</label>
                            <input class="form-control" type="text"name="news_author" value="<?php echo $result_getNewsInfor['news_author']?>">
                        </div>
                        <div class="form-group">
                            <label for="name">Thời gian</label>
                            <input class="form-control" type="date"name="news_time" value="<?php echo $result_getNewsInfor['news_time']?>">
                        </div>
                        <div class="form-group">
                            <label for="brand_id">Danh mục</label>
                            <select class="form-control" id="news_cateID" name="news_cateID" >
                                <?php
                                    foreach($result_showNewsCate as $row) {
                                    ?>
                                        <option value="<?php echo $row['news_cateID']?>" <?php if($result_getNewsInfor['news_cateID'] == $row['news_cateID']) echo "selected";?> >
                                            <?php echo $row['news_cateName']?>
                                        </option>
                                    <?php        
                                    }
                                ?>
                    ...
                    <?php
                        if(isset($_POST['btn_modify'])) {
                            ...
                            if($_FILES['news_img']['name'] == "") {
                                $sql_update = "UPDATE news SET news_title='$title_news', news_intro='$intro_news', news_content='$content_news', news_author='$author_news', news_time='$time_news', news_cateID='$cateID_news' WHERE news_id= $newsID";
                                ...
                                $sql_update = "UPDATE news SET news_title='$title_news', news_images='$img_news', news_intro='$intro_news', news_content='$content_news', news_author='$author_news', news_time='$time_news', news_cateID='$cateID_news'  WHERE news_id= $newsID";

                                if ($size_img <= 1000000 && (in_array($type_img, $allowed_filetypes))) {
                                    // Đẩy ảnh client->server
                                    move_uploaded_file($tmp_img, "asset/images/news/".$img_pro);

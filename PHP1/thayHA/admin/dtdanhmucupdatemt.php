<?php
	...
	if (isset($_POST['d1'])&&isset($_POST['d2'])&&isset($_POST['d3'])&&isset($_POST['d4'])){
		...
		if (mysqli_query($connect,"insert into modun(mamodun,tenmodun,batdau,ketthuc,makythi)
			values('".$_POST['d1']."','".$_POST['d2']."','".$_POST['d3']."','".$_POST['d4']."','".$_SESSION['kythi']."')")){
				echo "true";
				$hv=mysqli_query($connect,"select sbd from hocvien where makythi='".$_SESSION['kythi']."'");
				while ($ro=mysqli_fetch_array($hv)){
					mysqli_query($connect,"insert into allowexam(sbd,mamodun,allow) values('".$ro['sbd']."','".$_POST['d1']."','C')");
			}
		} else echo "false";
	}else echo "false";
?>